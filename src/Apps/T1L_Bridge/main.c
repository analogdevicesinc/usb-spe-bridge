...
#ifdef HAS_ADIN2111
    [IFACE_T1L] = "ADIN2111 (T1L)",
#else
    [IFACE_T1L] = "ADIN1110 (T1L)",
#endif
    [IFACE_USB] = "USB",
};
...
static void ADIN1110_Handler(net_pkt_t *pkt)
{
    PktHandler(IFACE_T1L, pkt);
}
...
void BackgroundTaskBody(void *pvParameters)
{
    ...
    //T1L is always Tx/Rx
    ifaceModes[IFACE_T1L] = IFACE_MODE_TXRX;
    ConfigureUSB_Mode(runtimeAppConfig.usbSniffer);


    if (ifaceModes[IFACE_T1L] != IFACE_MODE_OFF) {
        ADIN1110_Init(ADIN1110_Handler);
    }
