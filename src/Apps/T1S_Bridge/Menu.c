...
static void SetupMainMenu( )
{
    ...
    entry = &mainMenuEntries[numMenuEntries];
    entry->entryKey = "ad3306";
    entry->itemText = "AD3306 Debug";
    entry->helpText = "Opens the AD3306 sub-menu for debug commands specific to the AD3306.";
    entry->dataType = MENU_ITEM_SUBMENU;
    entry->data.submenu_t_data.submenu = AD3306_GetDebugMenu();
    numMenuEntries++;
